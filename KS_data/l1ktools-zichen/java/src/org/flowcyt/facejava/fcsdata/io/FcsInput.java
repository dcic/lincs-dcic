 ...
 * that can read FCS data files and produce FCSData impls. For each FCS input library, a class
 * implementing this interface is created. The class will use the library's methods to extract
 * the data that is needed by the FSCData impls, create the FCSData impls and return them
