...
/**
 * <p>
 * The default FcsInput. This adapter uses the CFCS library to read FCS files. (Thus, it depends on
 * CFCS being in the classpath.)
 * 
 * @author echng
 */
public class CFCSInput {
    ...
    /**
     * Uses CFCS to extract the FCS data needed by the FCSData impls.
     * 
     * @param readSystem
     *            The main CFCS class that represents a single data file.
     * @return Returns the FcsDataFile representation of the FCS data in the given CFCSSystem.
     * @throws InvalidDataSetsException
     *             Thrown if there was an error loading one of the data sets in the FCS file. The
     *             list returned by getDataSets() will not contain these DataSets. e.g., if Data Set
     *             0 and 2 are OK but 1 is not List Mode, only DataSets 0 and 2 will appear in the
     *             list. Use getReasons() to find out which data sets (by index) had errors and what
     *             the error was (e.g., it had a duplicate parameter name, it wasn't in List Mode,
     *             etc.)
     */
    ...
    /**
     * Extracts the FCS Data in the CFCSDataSet and creates the corresponding FCSData impls.
     * 
     * @param cfcsDs
     ...
     *             Thrown if one of the FCS parameters has an invalid parameter number.
     * @throws DuplicateParameterReferenceException
     *             Thrown if there is a duplicate FCS parameter name ($PnN) in the given
     *             CFCSDataSet.
     */
    private FcsDataSet extractDataSet(CFCSDataSet cfcsDs, int datasetNumber)
	    throws InvalidCFCSDataSetTypeException, InvalidParameterNumberException,
	    DuplicateParameterReferenceException {
